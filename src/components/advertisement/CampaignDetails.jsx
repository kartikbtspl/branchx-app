...
const CampaignDetails = () => {
  ...
  const isActionTaken = campaign?.isApproved === "APPROVE" || campaign?.isApproved === "REJECT";
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
        <div>
          ...
          <p className="text-gray-600 mt-2 max-w-3xl">{campaign.campaignDescription}</p>
        </div>
        <div className="mt-4 md:mt-0">
          <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(campaign.isApproved)}`}>
            {campaign.isApproved}
          </span>
        </div>
      </div>
      ...
      {actionStatus && (
        <div className={`mb-6 p-3 rounded-lg text-center ${
          actionStatus.includes("successfully") ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"
        }`}>
          {actionStatus}
        </div>
      )}
            ...
            <div className="border border-gray-200 rounded-lg p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{parseArray(campaign.ageGroups).join(", ") || "N/A"}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Regions</p>
                  <p className="font-medium">{parseArray(campaign.targetRegions).join(", ") || "N/A"}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium">{parseArray(campaign.selectedDays).join(", ") || "N/A"}</p>
                </div>
              </div>
            </div>
            ...
            <div className="border border-gray-200 rounded-lg p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-blue-50 p-4 rounded-lg">
                  ...
                  <p className="text-xl font-bold">{formatCurrency(campaign.baseBid)}</p>
                </div>
                <div className="bg-indigo-50 p-4 rounded-lg">
                  <p className="text-sm text-indigo-600">Max Bid Cap</p>
                  <p className="text-xl font-bold">{formatCurrency(campaign.maxBidCap)}</p>
                </div>
                <div className="bg-purple-50 p-4 rounded-lg">
                  <p className="text-sm text-purple-600">Budget Limit</p>
                  <p className="text-xl font-bold">{formatCurrency(campaign.budgetLimit)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="border border-gray-200 rounded-lg p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="font-medium">{formatDate(campaign.scheduleDate)}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">End Date</p>
                  <p className="font-medium">{formatDate(campaign.scheduleEndDate)}</p>
                </div>
                <div>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                    campaign.status ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                  }`}>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                    campaign.isPayment ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"
                  }`}>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(campaign.isApproved)}`}>
                    {campaign.isApproved}
                  </span>
                </div>
              </div>
